...
func (r *CategoryRepository) GetByID(ctx context.Context, id int64) (*domain.Category, error) {
	row := r.pool.QueryRow(ctx,
		`SELECT `+catCols+` FROM categories WHERE id = $1`+catNotDeleted, id)
...
func (r *CategoryRepository) Update(ctx context.Context, category *domain.Category) error {
	_, err := r.pool.Exec(ctx,
		`UPDATE categories SET name=$1, parent_id=$2, updated_at=$3 WHERE id=$4 AND deleted_at IS NULL`,
		category.Name, category.ParentID, category.UpdatedAt, category.ID)
	return err
}

func (r *CategoryRepository) Delete(ctx context.Context, id int64) error {
	_, err := r.pool.Exec(ctx, `UPDATE categories SET deleted_at=NOW() WHERE id=$1 AND deleted_at IS NULL`, id)
	return err
}

func (r *CategoryRepository) Restore(ctx context.Context, id int64) (*domain.Category, error) {
	_, err := r.pool.Exec(ctx, `UPDATE categories SET deleted_at=NULL WHERE id=$1`, id)
	if err != nil {
		return nil, err
	}
