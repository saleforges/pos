...
const paymentCols = `id, merchant_id, order_id, gateway, status, amount, payment_url, session_id, gateway_ref, created_at, updated_at`
...
func (r *PaymentRepository) Create(ctx context.Context, p *domain.PaymentTransaction) error {
	return r.pool.QueryRow(ctx,
		`INSERT INTO payment_transactions (merchant_id, order_id, gateway, status, amount, payment_url, session_id, gateway_ref, created_at, updated_at)
...
func scanPayment(row pgx.Row) (*domain.PaymentTransaction, error) {
	...
	var paymentURL, sessionID, gatewayRef *string
	err := row.Scan(&p.ID, &p.MerchantID, &p.OrderID, &p.Gateway, &p.Status, &p.Amount,
		&paymentURL, &sessionID, &gatewayRef, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
